...
import { FaComments, FaTimes, FaUserCog, FaReply } from 'react-icons/fa'
...
const NOTIF_SOUND_KEY = 'radio-l20-chat-sound'
...
export function FloatingChat() {
  ...
  }, [messages.length, isOpen, soundEnabled])
  ...
  const toggleSound = useCallback(() => {
    setSoundEnabled((prev) => {
      const next = !prev
      try { localStorage.setItem(NOTIF_SOUND_KEY, String(next)) } catch { /* */ }
      return next
    })
  }, [])
  ...
  return (
    <>
      <div className="floating-chat-wrapper">
        ...
        {!isOpen && unread > 0 && <span className="floating-chat-dot" />}
      </div>
        ...
        <div className="floating-chat-panel">
          <div className="floating-chat-header">
            <div className="floating-chat-header-info">
              <FaComments />
              <h3>Chat da Rádio L20</h3>
            </div>
            <div className="floating-chat-header-actions">
              {user && (
                <button
                  className={`floating-chat-settings-btn${showSettings ? ' active' : ''}`}
                  type="button"
          ...
          {!user ? (
            <div className="floating-chat-login">
              <p>Faça login para participar do chat.</p>
              <Link className="section-link" to={radioRoutes.register} onClick={() => setIsOpen(false)}>
                Cadastro ou login
              </Link>
            </div>
          ) : showSettings ? (
            <div className="floating-chat-settings">
              ...
              <div className="chat-settings-row">
                <label className="chat-sound-toggle">
                  <input
                    type="checkbox"
            ...
            <>
              <div className="floating-chat-messages">
                ...
                {messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`chat-bubble${isOwn ? ' is-own' : ''}`}>
                      {msg.photoURL ? (
                        <img className="chat-avatar" src={msg.photoURL} alt="" referrerPolicy="no-referrer" />
                      ) : (
                      ...
                      <div className="chat-bubble-body">
                        <div className="chat-bubble-meta">
                          <strong
                            className={`chat-bubble-name${badge ? ' is-highlighted' : ''}`}
                          >
                            {msg.displayName}
                          </strong>
                      ...
                      <div className="chat-bubble-actions">
                        ...
                        {isOwn && (
                          <button className="chat-delete-btn" type="button" onClick={() => handleDelete(msg.id)} aria-label="Excluir mensagem">
                            ✕
                          </button>
                        )}
                      </div>
              ...
              <form className="floating-chat-input-row" onSubmit={(e) => { e.preventDefault(); handleSend() }}>
                <textarea
                  ref={inputRef}
                  ...
                  disabled={sending}
                />
                <button className="floating-chat-send" type="submit" disabled={sending || !input.trim()} aria-label="Enviar">
                  {sending ? '...' : '➤'}
                </button>
              </form>

              {rateLimited && <p className="chat-error">Aguarde alguns segundos.</p>}
